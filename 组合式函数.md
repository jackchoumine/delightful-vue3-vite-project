...
![](./counter20221214091121.png)

### 修改 hook 返回的响应式数据

hook 返回的响应式数据，可在组件里修改，然后触发 hook 内部的逻辑执行。
...
`ref`函数的参数是 ref，返回 ref，是普通数据，就将其包裹成 ref。
...
了解上面 hook 编写技巧，在封装第三方库的使用，充分使用才能发挥威力。
...
const items = ref<Cart[]>([]) // 状态提取到hook之外
...
composition api 能在条件语句在使用，组合函数一样能在条件语句内使用。
...
> 注意 result 和 filterKey，不在条件语句声明，否则模板访问不到。
