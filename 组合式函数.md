...
在讨论具体的方式之前，我们先看看状态和状态逻辑的区别。

> 状态 vs 状态逻辑

状态即组件状态：影响组件 UI 层的数据，可理解成 props 和 state，它们往往是变量，是数据，不含函数逻辑。为何说往往？有时候函数也可以作为 prop 传入组件，较少。

状态逻辑：携带组件状态的**函数**，这些函数比较特殊，它们的执行结果会影响组件 UI，能在组件之间复用。
...
> setup 函数在组件创建时**只会执行一次**，并建立状态（数据）和逻辑（函数），状态和模板之间的连接。
...
- 组合函数会在第一次执行后建立状态之间的连接，即在 setup 中输入和输出建立连接
...
> 使用 `toRef`可从响应式对象中获取响应式属性，然后把它传递给组合函数。
...
1. MaybeRef ref unref

2. 将 hook 的输入（参数）和现有 ref 建立连接，即建立依赖关系

3. 将 hook 的输出（返回）和现有 ref 建立连接

```js
const { user } = useHttp({ name: 'jack' })
const age = computed(() => use.age)

// 相当于
const user =  = reactive({})
const age = computed(() => user.age)
http({name:'jack'}).then(res=>{
...
## 类型安全的 provide 和 inject

## reactive 状态共享

## useDemi 创建兼容 vue2 和 vue3 的库
